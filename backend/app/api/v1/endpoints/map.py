...
def get_map_hotspots(current_user: CurrentUser, db: Session = Depends(get_db)):
    """
    ...
    visible_village_ids = get_village_ids_in_scope(
        db, jurisdiction_id, role, jurisdiction_type
    )
    ...
    villages = (
        db.query(Jurisdiction).filter(Jurisdiction.id.in_(visible_village_ids)).all()
    )
    ...
    for village in villages:
        ...
        feature = {
            ...
            "geometry": {
                "type": "Point",
                "coordinates": [float(village.lon), float(village.lat)],  # GeoJSON is [lon, lat]
            },
            "properties": {
                "id": village.id,
                "name": village.name,
