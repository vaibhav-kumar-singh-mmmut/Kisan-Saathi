...
class OTPRequest(BaseModel):
    ...
    phone: str = Field(
        ..., description="Registered phone number (E.164 or local format)"
    )
...
class SignupRequest(BaseModel):
    ...
    user_type: str = Field(..., description="'farmer' or 'official'")
    role: Optional[str] = Field(None, description="Role (e.g. DM, Tehsildar)")
    wing: Optional[str] = Field(None, description="Wing (revenue, development, etc)")
    jurisdiction_type: Optional[str] = Field(None, description="Jurisdiction type")
