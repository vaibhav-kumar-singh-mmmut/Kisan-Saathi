...
def setup_test_db():
    ...
    with Session(_TEST_ENGINE) as session:
        session.add_all([
            DiseaseLookup(
                id="wheat_yellow_rust", name="Yellow Rust", crops=["Wheat"],
                severity="high", pathogen_type="fungal",
                ipm_steps=["Use resistant/tolerant wheat varieties where available", "Avoid excess nitrogenous fertilizer", "Spray recommended fungicide (e.g. Propiconazole) at first sign"]
            ),
            DiseaseLookup(
                id="rice_tungro", name="Rice Tungro", crops=["Rice"],
                severity="high", pathogen_type="viral",
                ipm_steps=["This shouldn't be returned"]
            ),
            DiseaseLookup(
                id="pomegranate_nematode_wilt", name="Pomegranate Nematode Wilt", crops=["Pomegranate"],
                severity="high", pathogen_type="nematode",
...
async def test_advisory_low_confidence(app_with_test_db):
    async with AsyncClient(transport=ASGITransport(app=app_with_test_db), base_url="http://test") as ac:
        response = await ac.get("/api/v1/advisory", params={"disease_id": "wheat_yellow_rust", "confidence": 0.65})
...
async def test_advisory_fungal(app_with_test_db):
    async with AsyncClient(transport=ASGITransport(app=app_with_test_db), base_url="http://test") as ac:
        response = await ac.get("/api/v1/advisory", params={"disease_id": "wheat_yellow_rust", "confidence": 0.85})
...
async def test_advisory_viral(app_with_test_db):
    async with AsyncClient(transport=ASGITransport(app=app_with_test_db), base_url="http://test") as ac:
        response = await ac.get("/api/v1/advisory", params={"disease_id": "rice_tungro", "confidence": 0.90})
...
async def test_advisory_nematode(app_with_test_db):
    async with AsyncClient(transport=ASGITransport(app=app_with_test_db), base_url="http://test") as ac:
        response = await ac.get("/api/v1/advisory", params={"disease_id": "pomegranate_nematode_wilt", "confidence": 0.95})
...
async def test_advisory_not_found(app_with_test_db):
    async with AsyncClient(transport=ASGITransport(app=app_with_test_db), base_url="http://test") as ac:
        response = await ac.get("/api/v1/advisory", params={"disease_id": "unknown_disease", "confidence": 0.90})
